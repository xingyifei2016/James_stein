...
#Given input shape, kernel_size, stride and output_channel
#Calculate the shape of the output
def calc_next(inputs, kern, stride, out_channel):
    if type(kern) == int:
        dims = int(math.floor((inputs-(kern-1)-1)/stride + 1))
        return torch.Size([2, out_channel, dims, dims])
    else:
        dims = int(math.floor((inputs-(kern[0]-1)-1)/stride + 1))
        return torch.Size([2, out_channel, dims, 1])

#The Shrinkage-Augmented Network
...
def main():
    ...
    parser.add_argument('--num-distr', type=int, default=5, metavar='N',
